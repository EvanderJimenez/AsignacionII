...
// Crea el validador
const validatePokemon = [check("pokemon").isString().notEmpty()];

export default async function listGifs(
  req: NextApiRequest,
  res: NextApiResponse
) {
  ...
  helmet()(req, res, async () => {
    ...
    jsonParser(req, res, async () => {
      if (req.method === "POST") {
        ...
        await Promise.all(
          validatePokemon.map((validation) => validation.run(req))
        );
        ...
        try {
          ...
          const giphyResponse = await fetch(
            `https://api.giphy.com/v1/gifs/search?api_key=${"z3c3q45cNAYqpVcu7BGbUWLsxAlJCGEk"}&q=Pokemon+${typeName}+combat&limit=10`
          );
          const giphyData = await giphyResponse.json();

          const gifs = giphyData.data.map(
